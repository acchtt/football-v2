(() => {
  ...
  async function showPwaSheet(mode='install'){
    ...
    else if(isAndroid()&&isLikelyWebView())instructions='This appears to be an in-app browser. Open the page in Chrome or Samsung Internet first, then use Install app / Add to Home screen from the browser menu.';
    else if(isAndroid())instructions='Use the Install button when the browser offers it. If no native prompt appears, open the browser menu and choose Install app or Add to Home screen.';
    ...
    sheet.innerHTML=`<div class="pwaSheetCard" role="dialog" aria-modal="true" aria-labelledby="pwaSheetTitle"><h3 id="pwaSheetTitle">${mode==='push'?'Push notification status':'Install SlipTrace'}</h3><p>${instructions}</p><div class="pwaChecks">${checkRow('Secure HTTPS',d.secureContext?'Ready':'Unavailable',d.secureContext?'ok':'bad')}${checkRow('Service worker',d.serviceWorker==='ready'||d.serviceWorker==='registered'?'Ready':'Issue',d.serviceWorker==='ready'||d.serviceWorker==='registered'?'ok':'bad')}${checkRow('Web Push API',d.pushApi?'Supported':'Unsupported',d.pushApi?'ok':'warn')}${checkRow('Notification permission',d.notificationPermission,d.notificationPermission==='granted'?'ok':d.notificationPermission==='denied'?'bad':'warn')}${checkRow('Push subscription',d.subscription?'Active':'Not active',d.subscription?'ok':'warn')}${checkRow('Push backend',d.backend?'Ready':'Unavailable',d.backend?'ok':'bad')}${checkRow('Native install prompt',d.installPrompt?'Available':'Not offered',d.installPrompt?'ok':'warn')}</div><div class="pwaSheetActions"><button type="button" data-copy>Copy link</button><button type="button" class="primary" data-test ${d.subscription&&d.backend?'':'disabled'}>Send test push</button><button type="button" data-close>Close</button></div></div>`;

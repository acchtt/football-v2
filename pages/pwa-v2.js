(() => {
  ...
  async function syncPushSelection(){
    ...
    }catch(error){console.warn('ARC XI push sync failed',error);return {ok:false,reason:'sync',error};}
  }
  ...
  async function toggleFollow(id,title,href,button){
    ...
    try{
      ...
      else if(result.reason==='ios-install')toast('Match saved. Install ARC XI to Home Screen before enabling iPhone push.');
    ...
    }catch(error){console.warn('ARC XI push enable failed',error);toast('Match saved, but push setup failed. Open PWA status for details.');}
  ...
  async function showPwaSheet(mode='install'){
    ...
    let instructions='';
    if(isStandalone())instructions='ARC XI is already running as an installed web app.';
    ...
    else if(isAndroid())instructions='Open ARC XI in the full Chrome app, then use Chrome menu → Install and create shortcut → Install app. If the native prompt is available, the Install button above can trigger the same browser installer.';
  ...
  function injectInstallButton(){
    ...
    if(!btn){
      btn=document.createElement('button');btn.type='button';btn.className='pwaInstallBtn';btn.textContent='Install';btn.setAttribute('aria-label','Install ARC XI web app');
      btn.addEventListener('click',async()=>{
        if(isStandalone()){toast('ARC XI is already installed.');return;}
        if(installPrompt){
          const prompt=installPrompt;installPrompt=null;
          try{await prompt.prompt();const choice=await prompt.userChoice;if(choice?.outcome==='accepted')toast('Installing ARC XI…');else showPwaSheet('install');}
  ...
  window.addEventListener('appinstalled',()=>{installPrompt=null;injectInstallButton();toast('ARC XI installed.');});
  ...
  registerSW().then(()=>syncPushSelection()).catch(error=>console.warn('ARC XI service worker registration failed',error));
  getPushConfig(true);
  window.SlipTracePWA={
    diagnostics,

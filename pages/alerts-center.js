(() => {
  ...
  async function syncNow() {
    ...
    try {
      ...
      console.warn('ARC XI alert settings sync failed', error);
  ...
  function renderSetup(status) {
    ...
    if (d?.subscription && d?.backend) {
      return '<div class="alertsSetup is-active"><div><strong>Device alerts are active</strong><small>ARC XI can notify you even when the app is closed.</small></div><span aria-hidden="true">✓</span></div>';
    }
    ...
    else if (d?.notificationPermission === 'denied') { text = 'Notifications are blocked'; note = 'Allow notifications for ARC XI in your browser or site settings.'; disabled = true; }
    else if (d && !d.pushApi) { text = 'Web Push unsupported'; note = 'Open ARC XI in a browser that supports Web Push.'; disabled = true; }
  ...
  function enableFailureMessage(result) {
    ...
    if (result.reason === 'ios-install') return 'Install ARC XI to your Home Screen, open the installed app, then enable alerts.';

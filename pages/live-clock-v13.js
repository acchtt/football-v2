// SlipTrace Live Clock v13 — provider-first, period-safe match clock.
(() => {
  'use strict';
  ...
  function nameScore(a,b){
    ...
    aa.forEach(t=>bb.has(t)&&s++); const o=s/Math.max(aa.size,bb.size);
  ...
  function eventFor(match){
    ...
    return events().map(e=>({e,s:nameScore(t.home,e.home)+nameScore(t.away,e.away)}))
  ...
  function periodNum(v){
    const p=String(v??'').trim().toLowerCase().replace(/[^a-z0-9]+/g,'');
  ...
  function periodLabel(p,status){
    if(FINISHED.has(String(status||'').toLowerCase())) return 'FT';
    if(p===1) return '1H'; if(p===2) return '2H'; if(p===3) return 'ET1'; if(p===4) return 'ET2';
    return String(status||'LIVE').toUpperCase();
  }
  function offsetForPeriod(p){ return p===2?45:p===3?90:p===4?105:0; }

  function unwrapDetail(){
    const raw=window.SLIPTRACE_MATCH_CONTEXT?.detail;
    if(!raw) return null;
    return isObj(raw.event) ? {...raw.event,...raw} : raw;
  }
  ...
  function clockSource(source,kickoff,anchorAt){
    ...
    // BSD WebSocket/detail minute is full-match time. Some compact REST rows have
    // historically appeared period-relative. Never overwrite a supplied minute with
    // period_started_at_uts; use that timestamp only when no minute exists at all.
    if(minute===undefined && started!==undefined && p){
      const elapsed=Math.max(0,Math.floor(Date.now()/1000-started));
      minute=offsetForPeriod(p)+Math.floor(elapsed/60);
      second=elapsed%60;
    }
    ...
    // Only add a period offset when the supplied minute is clearly period-relative.
    if(p===2 && minute<45) minute+=45;
    else if(p===3 && minute<90) minute+=90;
    else if(p===4 && minute<105) minute+=105;
    ...
    const age=Math.max(0,Math.min(8,Math.floor((Date.now()-(anchorAt||Date.now()))/1000)));
    if(!paused && started===undefined) second+=age;
    minute+=Math.floor(second/60); second%=60;

    return {
      label:`${Math.max(0,minute)}:${String(second).padStart(2,'0')}`,
      status:periodLabel(p,status), period:p, rawMinute, finished:false,
    };
  }

  function bestClock(row,event,isCurrentMatch=false){
    if(isCurrentMatch){
      const detail=unwrapDetail();
      const c=clockSource(detail,row?.kickoff||row?.displayKickoff,detailAt);
      if(c) return c;
    }
  ...
  function updateList(){
    ...
    document.querySelectorAll('details.fixtureRow[data-row-key]').forEach(node=>{
      const row=findRow(node.dataset.rowKey,d.schedule), event=row?eventFor(row.match):null;
      if(!row||!event) return;
    ...
    document.querySelectorAll('details.pickRow[data-row-key]').forEach(node=>{
      const row=findRow(node.dataset.rowKey,d.picks), event=row?eventFor(row.match):null;
      if(!row||!event) return;
      const c=bestClock(row,event,false), score=node.querySelector('.score'); if(!c||!score) return;
      const strong=score.querySelector('strong'), span=score.querySelector('span');
      if(strong&&event.homeScore!==undefined&&event.awayScore!==undefined) strong.textContent=`${event.homeScore}–${event.awayScore}`;
  ...
  function field(name){
    return [...document.querySelectorAll('#matchApp .liveData .matchField')]
      .find(n=>n.querySelector('span')?.textContent.trim().toLowerCase()===name.toLowerCase());
  }
  function setField(name,value){ const s=field(name)?.querySelector('strong'); if(s) s.textContent=value; }

  function updateMatch(){
    if(matchApp.hidden||!/^#match\//.test(location.hash)) return;
    const d=dashboard(); if(!d) return;
    ...
    if(board&&event.homeScore!==undefined&&event.awayScore!==undefined){
      const bs=board.querySelectorAll('b'); if(bs.length>=2){bs[0].textContent=event.homeScore;bs[1].textContent=event.awayScore;}
    }

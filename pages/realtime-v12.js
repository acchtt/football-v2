// SlipTrace Realtime v12 — uses BSD live key while REST panels use canonical event id.
(() => {
  'use strict';
  ...
  function liveId(){return String(window.SLIPTRACE_MATCH_CONTEXT?.liveId||matchApp.querySelector('.matchTopline')?.dataset.liveId||'').trim()||null;}
  ...
  function frame(f){if(!f||typeof f!=='object')return;if(f.event_id&&String(f.event_id)!==String(state.id))return;if(f.type==='subscribed'){state.source=f.source||'basic';state.blocked=false;setCoverage(state.source==='full'?'WS+':'LIVE WS',state.source==='full'?'plus':'live',state.source==='full'?'per-action position':'position ~5s');const h=Array.isArray(f.history)?f.history:[],l=Array.isArray(f.livedata)?f.livedata:[];if(h.length)action(h[h.length-1]);else if(l.length)livedata(l[l.length-1]);else apply();return;}if(f.type==='livedata'){setCoverage(state.source==='full'?'WS+':'LIVE WS',state.source==='full'?'plus':'live',state.source==='full'?'per-action position':'position ~5s');livedata(f);return;}if(f.type==='action'){state.source='full';setCoverage('WS+','plus','per-action position');action(f);return;}if(f.type==='error'){const code=String(f.code||''),msg=f.message||code||'WebSocket error';if(code==='subscription_required'){state.blocked=true;state.situation='WebSocket addon required';state.commentary='Real pitch animation requires the BSD WebSocket addon. REST stats remain active.';setCoverage('WS ADDON','limited','BSD WebSocket addon required');}else if(code==='not_tracked'){state.blocked=true;state.situation='No positional feed';state.commentary='BSD does not provide positional WebSocket coverage for this match.';setCoverage('NO WS','limited','no positional coverage');}else{state.situation='WebSocket error';state.commentary=msg;setCoverage('WS ERROR','limited',msg);}apply();}}
  ...
  function connect(){if(!state.id||state.socket||state.blocked||!active()||document.visibilityState==='hidden')return;const id=state.id;setCoverage('CONNECTING','stats','opening BSD live channel');let s;try{s=new WebSocket(wsUrl());}catch(e){setCoverage('WS ERROR','limited',e?.message||'connection failed');reconnect(id);return;}state.socket=s;s.addEventListener('open',()=>{if(String(id)!==String(state.id))return;try{s.send(JSON.stringify({action:'subscribe',event_id:id}));}catch{}});s.addEventListener('message',e=>{if(String(id)!==String(state.id))return;let f;try{f=JSON.parse(e.data);}catch{return;}frame(f);});s.addEventListener('close',()=>{if(state.socket===s)state.socket=null;if(String(id)!==String(state.id)||state.blocked)return;setCoverage('RECONNECT','limited','BSD live channel disconnected');reconnect(id);});s.addEventListener('error',()=>{if(String(id)===String(state.id)&&!state.blocked)setCoverage('WS ERROR','limited','live channel unavailable');});}
  function sync(){if(!active()){close();state.id=null;return;}const id=liveId();if(!id)return;if(String(id)!==String(state.id)){close();state.id=id;state.blocked=false;state.source='';state.position=null;state.situation='';state.commentary='';state.activity=[];state.coverage=null;}connect();apply();}
